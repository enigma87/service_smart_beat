...
function QVTrainingSessionHistory(userid) {
	...
	var csvdata = arrayToCSVString(beanArrayToCSVArray(response.obj.trainingSessions));
	...
	*/	
	...
	$('#exporttrainingsesisonslink').bind("click" ,function() {
        ...
        for (var i = 0; i < response.obj.trainingSessions.length; i++) {

	var trainingSessionBean = response.obj.trainingSessions[i];
            ...
            if (response.obj.trainingSessions.length > (i + 1)) {
